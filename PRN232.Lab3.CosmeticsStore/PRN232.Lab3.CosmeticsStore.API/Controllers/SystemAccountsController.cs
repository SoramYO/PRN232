...
using PRN232.Lab3.CosmeticsStore.Service.Payload.Response;
...
namespace PRN232.Lab3.CosmeticsStore.API.Controllers
{
	...
	public class SystemAccountsController : ControllerBase
	{
		...
		[HttpPost("Login")]
		public async Task<ActionResult> Login([FromBody] SystemAccountRequest request)
		{
			try
			{
				var account = await _systemAccountService.Login(request);
				if (account == null)
				{
				...
				return Ok(new SystemAccountResponse
				{
					Role = role,
